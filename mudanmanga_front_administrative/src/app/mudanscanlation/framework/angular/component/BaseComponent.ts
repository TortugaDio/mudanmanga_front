...
import { DominioMensajeUsuario } from '../../modelo/generico/DominioMensajeUsuario';
import { UIMantenimientoController } from '../interface/UIMantenimientoController';
import { ConstanteAngular } from '../ConstanteAngular';
import { convertDateStringsToDates } from '../funciones/dateutils';
import { SeguridadUsuarioActual } from '../../modelo/seguridad/SeguridadUsuarioActual';
...
export class BaseComponent {
    ...
    mostrarMensajeAdvertenciaNoClearTipo(mensaje: string, tipo: string): void {
        this.mostrarMensajeNoClear(mensaje, tipo);
    }
    ...
    mostrarMensajeAdvertenciaEnGrillaNoClear(mensaje: string, linea: number): void {
        this.mostrarMensajeEnGrilla(mensaje, 'warn', linea);
    }
    mostrarMensajeEnGrilla(mensaje: string, tipo: string, linea: number): void {
        this.messageService.add({ severity: tipo, summary: 'Error en la Línea ' + linea, detail: mensaje, life: 25000 });
    }
    ...
    get TIPOSBOTONES_MANTENIMIENTO() {
        return {
            ...
            BAS2BS: tiposBotonesMantenimiento.BAS2BS
        }
    }
    ...
    contenidoFileValidoPorTipos(base64: string, tiposPermitidos: string[], nombrearchivo?: string): boolean {
        var valida = false;

        var extension = '';
        var validaextension = false;

        if (!this.estaVacio(nombrearchivo)) {

            var lst = nombrearchivo.split(".");
            let lastItem = lst[lst.length - 1];
            extension = lastItem.toLocaleUpperCase();
